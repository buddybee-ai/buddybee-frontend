...
// NOTE: uses a static class-name map (not template-literal interpolation) so
// Tailwind's JIT scanner can actually detect and generate these classes.
const METRIC_STYLES = {
  emerald: { bg: 'bg-emerald-50', border: 'border-emerald-100', text: 'text-emerald-700', bar: 'bg-emerald-500' },
  primary: { bg: 'bg-primary-50', border: 'border-primary-100', text: 'text-primary-700', bar: 'bg-primary-500' },
  purple:  { bg: 'bg-purple-50',  border: 'border-purple-100',  text: 'text-purple-700',  bar: 'bg-purple-500' },
  rose:    { bg: 'bg-rose-50',    border: 'border-rose-100',    text: 'text-rose-700',    bar: 'bg-rose-500' },
}
...
function EngagementMetrics({ students }) {
  ...
  const metrics = [
    ...
    { label: 'AI Engagement', value: `${engagementPct}%`, color: 'primary', bar: engagementPct },
  ...
  return (
    <div className="card">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="font-display font-bold text-slate-800 text-sm">Platform Engagement Metrics</h3>
      </div>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {metrics.map((m, i) => {
          const c = METRIC_STYLES[m.color] ?? METRIC_STYLES.primary
          return (
            <motion.div
              key={m.label}
            ...
            >
              <div className={`font-display text-2xl font-bold ${c.text}`}>{m.value}</div>
              ...
              {m.bar !== null && (
                <div className="mt-2 h-1.5 bg-white rounded-full overflow-hidden">
                  <motion.div
                    initial={{ width: 0 }}
                    whileInView={{ width: `${Math.min(m.bar, 100)}%` }}
                    viewport={{ once: true }}
      ...
      <div className="p-6 space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" placeholder="e.g. City School" value={schoolName}
              ...
              className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm bg-slate-50 focus:outline-none focus:border-slate-400 focus:ring-4 focus:ring-slate-500/10" />
          </div>
          <div>
        ...
        <div>
          ...
          <input type="text" placeholder="e.g. CITY_SCHOOL_SADDAR" value={schoolId}
            ...
            className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm bg-slate-50 font-mono focus:outline-none focus:border-slate-400 focus:ring-4 focus:ring-slate-500/10" />
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
...
export default function AdminDashboard() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center min-h-[60vh] p-10">
      <div className="text-center"><div className="text-4xl mb-3 animate-bounce-soft">🐝</div><p className="text-slate-500 text-sm">Loading Admin Dashboard...</p></div>
    </div>
  )
  ...
  return (
    <div className="p-2 sm:p-3 max-w-7xl mx-auto space-y-5">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
        className="relative overflow-hidden bg-gradient-to-br from-ink-900 via-ink-800 to-slate-900 text-white p-6 rounded-2xl shadow-glow-lg"
      >
        <div className="absolute -top-16 -right-10 w-56 h-56 bg-primary-500/20 rounded-full blur-3xl" />
        ...
        <div className="relative flex flex-wrap gap-3 mt-4">
          {[
            ['Students', students.length, 'text-white'],
            ['Branches', schools.length, 'text-white'],
            ['Counselors', counselors.length, 'text-white'],
            ['High Risk', students.filter(s => s.risk_level === 'high').length, 'text-rose-400'],
            ...
            <motion.div
              ...
              className="bg-white/10 backdrop-blur-sm border border-white/10 rounded-xl px-4 py-2 text-center"
            >
      ...
      <AnimatePresence>
        {tab === 'overview' && (
          <motion.div key="overview" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-5">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <StatsCard title="Students"   value={students.length}        icon={Users}         color="blue"   delay={0} />
              <StatsCard title="Branches"   value={schools.length}         icon={School}        color="purple" delay={0.05} />
              <StatsCard title="Alerts"     value={dashboard?.alerts || 0} icon={AlertTriangle} color="red"    delay={0.1} />
              <StatsCard title="Counselors" value={counselors.length}      icon={Briefcase}     color="green"  delay={0.15} />
            </div>
        ...
        {tab === 'schools' && (
          <motion.div key="schools" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-5">
        ...
        {tab === 'counselors' && (
          <motion.div key="counselors" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-5">
            <CreateCounselorForm schools={schools} onSuccess={fetchAll} />
            <div className="bg-white rounded-2xl border border-slate-100 shadow-card overflow-hidden">
              <div className="p-5 border-b border-slate-100 flex items-center gap-2">
                <Briefcase size={18} className="text-purple-500" />
                <h3 className="font-display font-bold text-slate-800">All Counselors</h3>
              ...
              {counselors.length === 0 ? (
                ...
                <div className="divide-y divide-slate-100">
                  {counselors.map((c, i) => (
                    <motion.div key={c.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }} className="p-4 flex items-center gap-4 hover:bg-slate-50 transition-colors">
                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-purple-400 to-primary-500 flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
        ...
        {tab === 'students' && (
          <motion.div key="students" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-5">
