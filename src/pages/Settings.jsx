...
import { Bell, Lock, User, Globe, Shield, Palette, Brain, LogOut, Save, Eye, EyeOff, MessageSquare, Info, CheckCircle2, AlertCircle } from "lucide-react"
...
export default function Settings(){
  ...
  return <div className="max-w-5xl mx-auto px-2 pb-10">
    ...
    <motion.div initial={{opacity:0,y:-8}} animate={{opacity:1,y:0}} className="mb-6">
      <h1 className="font-display text-2xl md:text-3xl font-bold text-slate-900">Settings</h1>
      <p className="text-sm text-slate-500 mt-1">Manage your BuddyBee account, security, preferences and privacy.</p>
    </motion.div>
    <AnimatePresence>
      {(message||error)&&<motion.div initial={{opacity:0,height:0}} animate={{opacity:1,height:"auto"}} exit={{opacity:0,height:0}} className="overflow-hidden">
        <div className={`mb-5 flex items-center gap-2 rounded-xl border p-3 text-sm font-semibold ${error?"bg-red-50 border-red-200 text-red-600":"bg-emerald-50 border-emerald-200 text-emerald-700"}`}>
    ...
    <Section icon={User} title="Account" description="Your personal account information.">
      <form onSubmit={saveProfile} className="grid md:grid-cols-2 gap-4">
        ...
        <div className="md:col-span-2 flex items-center justify-between flex-wrap gap-3 pt-2"><span className="text-xs text-slate-400">Role: <b>{user?.roleLabel||user?.role}</b></span><button className={primary}><Save size={15}/>Save profile</button></div>
      </form>
    </Section>
    ...
    <Section icon={Lock} title="Password & security" description="Change your password or recover access if you forget it.">
      ...
      <div className="mt-5 border-t pt-4 text-sm text-slate-500">Forgot your password? <a className="text-primary-600 font-semibold" href="/forgot-password">Send yourself a reset link.</a></div>
    </Section>

    <Section icon={Globe} title="Language & regional" description="Choose the language used throughout BuddyBee.">
      <div className="flex flex-wrap gap-2">
        {LANGUAGES.map(l=><button key={l.code} onClick={()=>{setLang(l.code);savePrefs({language:l.code})}} className={`px-4 py-2.5 rounded-xl border text-sm font-semibold ${lang===l.code?"border-primary-500 bg-primary-50 text-primary-700":"border-slate-200 text-slate-600 hover:bg-slate-50"}`}>{l.flag} {l.native}</button>)}
      </div>
    </Section>
    ...
    <Section icon={MessageSquare} title="Chat" description="Fine-tune how BuddyBee talks with you.">
      ...
      <div className="mt-4">
        <button
          ...
          className="text-xs font-semibold text-slate-500 hover:text-primary-600 underline underline-offset-2"
        >
    ...
    <Section icon={Info} title="About" description="App info and support.">
      <div className="text-sm text-slate-600 space-y-2">
        ...
        <div className="border-b border-slate-100 pb-2"><a href="#" className="text-primary-600 font-semibold">Privacy Policy</a></div>
        <div className="border-b border-slate-100 pb-2"><a href="#" className="text-primary-600 font-semibold">Terms of Service</a></div>
        <div className="pb-1"><a href="/contact" className="text-primary-600 font-semibold">Contact Support</a></div>
      </div>
    </Section>
...
const input="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-800 outline-none focus:border-primary-400 focus:ring-4 focus:ring-primary-500/10"
const primary="inline-flex items-center gap-2 rounded-xl bg-primary-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-primary-700 hover:-translate-y-0.5 transition-all duration-200 shadow-primary disabled:opacity-50"

function Section({icon:Icon,title,description,children}){return <motion.section initial={{opacity:0,y:12}} whileInView={{opacity:1,y:0}} viewport={{once:true,margin:"-40px"}} transition={{duration:0.4}} className="bg-white border border-slate-100 rounded-2xl shadow-card p-5 md:p-6 mb-4"><div className="flex gap-3 mb-5"><div className="w-10 h-10 rounded-xl bg-primary-50 text-primary-600 flex items-center justify-center shrink-0"><Icon size={19}/></div><div><h2 className="font-display font-bold text-slate-900">{title}</h2><p className="text-xs text-slate-500 mt-0.5">{description}</p></div></div>{children}</motion.section>}
...
function PasswordField({label,value,onChange,show,toggle}){return <Field label={label}><div className="relative"><input className={`${input} pr-11`} type={show?"text":"password"} value={value} onChange={e=>onChange(e.target.value)} autoComplete="current-password"/><button type="button" onClick={toggle} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-primary-600 transition-colors">{show?<EyeOff size={17}/>:<Eye size={17}/>}</button></div></Field>}
function Toggle({label,description,value,onChange}){return <div className="flex items-center justify-between gap-4 py-3 border-b last:border-0 border-slate-100"><div><div className="text-sm font-semibold text-slate-800">{label}</div><div className="text-xs text-slate-500 mt-0.5">{description}</div></div><button type="button" role="switch" aria-checked={value} onClick={()=>onChange(!value)} className={`w-11 h-6 rounded-full p-1 transition-colors duration-200 ${value?"bg-primary-600":"bg-slate-300"}`}><motion.span layout transition={{type:"spring",stiffness:500,damping:32}} className="block w-4 h-4 bg-white rounded-full shadow-sm" style={{marginLeft: value ? "20px" : "0px"}}/></button></div>}
